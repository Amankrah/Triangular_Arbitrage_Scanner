...
def structure_trading_pairs(pairs,limit):
    ...
    for pair_a in pairs_list:
        ...
        a_token_0_reserve = pair_a["reserve0"]
        a_token_1_reserve = pair_a["reserve1"]
        ...
        for pair_b in pairs_list:
            ...
            b_token_0_reserve = pair_b["reserve0"]
            b_token_1_reserve = pair_b["reserve1"]
            ...
            if a_pair != b_pair:
                if b_base in a_pair_box or b_quote in a_pair_box:
                    ...
                    for pair_c in pairs_list:
                        ...
                        c_token_0_reserve = pair_c["reserve0"]
                        c_token_1_reserve = pair_c["reserve1"]
                            ...
                            if counts_c_base == 2 and counts_c_quote == 2 and c_base != c_quote:
                                ...
                                if unique_string not in remove_duplicates_list:
                                    output_dict = {
                                        ...
                                        "areserve0": a_token_0_reserve,
                                        "areserve1": a_token_1_reserve,
                                        "breserve0": b_token_0_reserve,
                                        "breserve1": b_token_1_reserve,
                                        "creserve0": c_token_0_reserve,
                                        "creserve1": c_token_1_reserve
                                    }
...
def calc_triangular_arb_surface_rate(t_pair, min_rate):
    ...
    for direction in direction_list:
        ...
        # Set price info
        a_token_0_reserve = float(t_pair["areserve0"])
        a_token_1_reserve = float(t_pair["areserve1"])
        b_token_0_reserve = float(t_pair["breserve0"])
        b_token_1_reserve = float(t_pair["breserve1"])
        c_token_0_reserve = float(t_pair["creserve0"])
        c_token_1_reserve = float(t_pair["creserve1"])
        ...
        if direction == "forward":
            swap_1 = a_base
            swap_2 = a_quote
            swap_1_rate = (float(a_token_1_reserve) * float(starting_amount)) / (float(a_token_0_reserve) + float(starting_amount))
        ...
        if direction == "reverse":
            swap_1 = a_quote
            swap_2 = a_base
            swap_1_rate = (float(a_token_0_reserve) * float(starting_amount)) / (float(a_token_1_reserve) + float(starting_amount))
            pool_direction_trade_1 = "quoteToBase"

        # Place first trade
        pool_contract_1 = a_contract
        acquired_coin_t1 = float(swap_1_rate)

        # Forward: check if aQuote (acquired coin) matches bQuote //////////
        if direction == "forward":
            if a_quote == b_quote and calculated == 0:
                swap_2_rate = (float(b_token_0_reserve) * float(acquired_coin_t1)) / (float(b_token_1_reserve) + float(acquired_coin_t1))
                acquired_coin_t2 = float(swap_2_rate)
                ...
                if b_base == c_base:
                    swap_3 = c_base
                    swap_3_rate = (float(c_token_1_reserve) * float(acquired_coin_t2)) / (float(c_token_0_reserve) + float(acquired_coin_t2))
                ...
                if b_base == c_quote:
                    swap_3 = c_quote
                    swap_3_rate = (float(c_token_0_reserve) * float(acquired_coin_t2)) / (float(c_token_1_reserve) + float(acquired_coin_t2))
                    pool_direction_trade_3 = "quoteToBase"
                    pool_contract_3 = c_contract

                acquired_coin_t3 = float(swap_3_rate)
                calculated = 1

        # Forward: check if aQuote (acquired coin) matches bBase //////////
        if direction == "forward":
            if a_quote == b_base and calculated == 0:
                swap_2_rate = (float(b_token_1_reserve) * float(acquired_coin_t1)) / (float(b_token_0_reserve) + float(acquired_coin_t1))
                acquired_coin_t2 = float(swap_2_rate)
        ...
        if direction == "forward":
            if a_quote == c_quote and calculated == 0:
                swap_2_rate = (float(c_token_0_reserve) * float(acquired_coin_t1)) / (float(c_token_1_reserve) + float(acquired_coin_t1))
                acquired_coin_t2 = float(swap_2_rate)
                pool_direction_trade_2 = "quoteToBase"
                ...
                if c_base == b_base:
                    swap_3 = b_base
                    swap_3_rate = (float(b_token_1_reserve) * float(acquired_coin_t2)) / (float(b_token_0_reserve) + float(acquired_coin_t2))
                    pool_direction_trade_3 = "baseToQuote"
                ...
                if c_base == b_quote:
                    swap_3 = b_quote
                    swap_3_rate = (float(b_token_0_reserve) * float(acquired_coin_t2)) / (float(b_token_1_reserve) + float(acquired_coin_t2))
                    pool_direction_trade_3 = "quoteToBase"
                    pool_contract_3 = b_contract
        ...
        if direction == "forward":
            if a_quote == c_base and calculated == 0:
                swap_2_rate = (float(c_token_1_reserve) * float(acquired_coin_t1)) / (float(c_token_0_reserve) + float(acquired_coin_t1))
                acquired_coin_t2 = float(swap_2_rate)
                pool_direction_trade_2 = "baseToQuote"
                pool_contract_2 = c_contract
        ...
        if direction == "reverse":
            if a_base == c_base and calculated == 0:
                ...
                if c_quote == b_base:
                    swap_3 = b_base
                    swap_3_rate = (float(b_token_1_reserve) * float(acquired_coin_t2)) / (float(c_token_0_reserve) + float(acquired_coin_t2))
                    pool_direction_trade_3 = "baseToQuote"
                    pool_contract_3 = b_contract
        ...
        profit_loss = acquired_coin_t3 - amount
        profit_loss_perc = (profit_loss / amount) * 100 if profit_loss != 0 else 0

        # Format Description
        trade_description_1 = f"Start with {swap_1} of {amount}. Swap at {swap_1_rate} for {swap_2} acquiring {acquired_coin_t1}."
        ...
        if profit_loss_perc <= min_surface_rate and profit_loss_perc >= 1.5:
